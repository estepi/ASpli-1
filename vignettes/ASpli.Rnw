...
\subsection{ Defining the targets } \label{sec:targetsDef}
...
Results are stored into an \texttt{ASpliCounts} object. Read densities ( $\frac{ \mbox{number
of reads}} {\mbox{feature length}}$ ) are calculated for genes and bins.
Count tables and read densities tables are produced at different genomic levels,
including genes, bins, junctions, and intron flanking regions.
...
counts <- readCounts ( 
    ...
    minAnchor = NULL )
@
The method \texttt{readCounts} accepts several parameters:
