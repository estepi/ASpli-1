...
.filterDU <- function( aspliDU, what = c( 'genes','bins','junctions'), fdr = 1, 
  ...
  if ( all( tolower(what) %in% whatValidValues ) ) {
    ...
    for ( slotName in tolower(what) ) {
      ...
      if ( nrow( currentSlot) > 0 ) {
        ...
        warning( paste(slotName, 'from ASpliDU object is empty.')  )
    ...
    stop( "Argument 'what' contains invalid values. Valid values are: c( 'genes','bins','junctions')" )        
  }
