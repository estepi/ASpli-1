...
.makePlot <- function ( data, useBarPlot = FALSE, main, col, factorsAndValues, 
  ...
  xTicksLabels <- makeXticksLabels( justOneFactor , gridPanels, 
      factorsAndValues, mainFactorIndex )
...
.plotBins <- function( 
  ...
  if ( outputIsAValidFile ) {
    ...
    if ( is.na(deviceIndex) ) { 
      ...
      print( devicesNames[deviceIndex])
  ...
  .makePlot( data, useBarplots, main = "Bin counts", col = binCountsColor, 
      factorsAndValues, justOneFactor, gridPanels, nPoints, legendAtSide,
      las.x, panelTitleCex, panelTitleColor, useHCColors, barWidth, barSpacer )
  
  # Add Gene count data
  data <- .prepareData( geneCounts, gridPanels, targets, factorsAndValues )
  .makePlot( data, useBarplots, main = "Gene counts", col = geneCountsColor, 
      factorsAndValues, justOneFactor, gridPanels, nPoints, legendAtSide,
      las.x, panelTitleCex, panelTitleColor, useHCColors, barWidth, barSpacer )
  ...
  .makePlot( data, useBarplots, main = "PSI / PIR", col = psirColor, 
      factorsAndValues, justOneFactor, gridPanels, nPoints, legendAtSide,
      las.x, panelTitleCex, panelTitleColor, useHCColors, barWidth, barSpacer )
  
  # Add J1 Exc count data
  data <- .prepareData( J1, gridPanels, targets, factorsAndValues )
  .makePlot( data, useBarplots, main = "Incl. junc 1", col = j1CountsColor, 
      factorsAndValues, justOneFactor, gridPanels, nPoints, legendAtSide,
      las.x, panelTitleCex, panelTitleColor, useHCColors, barWidth, barSpacer )
  
  # Add J2 Exc count data
  data <- .prepareData( J2, gridPanels, targets, factorsAndValues )
  .makePlot( data, useBarplots, main = "Incl. junc 2", col = j2CountsColor, 
      factorsAndValues, justOneFactor, gridPanels, nPoints, legendAtSide,
      las.x, panelTitleCex, panelTitleColor, useHCColors, barWidth, barSpacer )
  
  # Add J3 Exc count data
  data <- .prepareData( J3, gridPanels, targets, factorsAndValues )
  .makePlot( data, useBarplots, main = "Exclu. junc", col = j3CountsColor, 
      factorsAndValues, justOneFactor, gridPanels, nPoints, legendAtSide,
      las.x, panelTitleCex, panelTitleColor, useHCColors, barWidth, barSpacer )
